...
def _agent_system_prompt() -> str:
    return (
        ...
        "memory.query_profile summarizes whether the task is latest/recency-led, year-range-led, by-subject, or topic-led; treat those as first-class planning constraints. "
        ...
        "memory.priority_direct_hits lists strong paper-detail hits already surfaced by search; for latest or semantic-topic queries, prefer those over giant listings, bibliographies, or PDFs unless an official listing page is still the clearly better next step. "
        ...
        "Use url/title/peek/page_role/page_family together to classify URL quality before choosing targets: official accepted/program pages first, then useful detail pages, and avoid mirrors, social/news/blog posts, or generic low-signal pages when an official page for the same query is already available. "
        ...
        "When multiple venue families are in memory, prefer one strong official representative per family before revisiting the same family or adding companions. "
