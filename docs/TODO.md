...
- `E10 -> H9, H10`: require replay/targeted validation after each refactor slice that moved ownership
  ...
  - next: split this into two separate follow-ups:
    ...
    - `E14`: canonical loop-state reconciliation before more extraction feature work
      ...
      - next: handle venue page families earlier in planner memory: accepted-papers pages, program/papers-info pages, proceedings pages, and venue homepages often contain redundant paper sets, so the planner should choose one representative first and only spend another extract turn on a companion page when it clearly fills missing fields
    ...
    - `E16`: narrow candidate-URL proposal to true next-page complements
      ...
      - strategy change: drop candidate-URL proposal during the main shortlisted-page extraction loop for now; it is adding complexity, late-cycle failures, and redundant-page churn without enough yield on the current HTML-first venue tasks
