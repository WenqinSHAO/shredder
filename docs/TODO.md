...
- End-to-end agentic search spec is maintained in `docs/AGENTIC_SEARCH.md`.
...
Deferred backlog:
- cross-source author canonicalization improvements
...
- `M-Agentic-Extract-QoS-v1`: in progress; main blockers are cycle-safe canonicalization, consistent `papers by X` semantics, complementary official-page fetch coverage, and replay-backed validation.
...
| A2 | Stateless extractor schema-first row extraction (`paper_title_normalized`, authors, affiliations, abstract, evidence) | In progress | Schema extraction is live; deterministic post-LLM rejection is being removed so `match` rows survive into canonicalization, while row completeness remains tracked for later enrichment. |
...
| A8 | Post-extract stateless LLM canonicalization pass (dedup/clean/verify, no new papers) | In progress | Implemented; canonicalizer is now the primary semantic cleanup stage, with remaining work around stronger merge quality and clearer user-facing audit. |
| A5 | Final artifact slimming: final result contains final matches only | In progress | Result artifact is slimmer; trajectory still needs better canonicalize/progress surfacing. |
...
| A12 | Fast-path batch extraction prototype for authoritative pages | In progress | Defaulting authoritative/listing pages toward page-scale extraction batches before final canonicalization to cut extractor latency. |
| A13 | User-facing trajectory surfacing for canonicalize/extract progress | In progress | Show canonicalizer keep/drop summaries, extract coverage, and accumulated final-match progress in trajectory. |
...
| Paper Context Retrieval | Open after agentic final candidates are consistently canonicalized. |
